...
import { LayoutDashboard, Users, Shield, MapPin, Briefcase, ClipboardList, Layers, Calendar, X, Bell, History, CalendarCheck, BarChart2 } from 'lucide-react';
import { useUser } from '../context/UserContext';
import { Skeleton } from '@mui/material';

const Sidebar = ({ isOpen, onClose }) => {
    const { hasPermissionAccess, hasPermission, loading, userInfo } = useUser();

    const userTypeId = userInfo?.userTypeId || localStorage.getItem('userType');
    const isEmployee = String(userTypeId) === '2';

    if (loading) {
        return (
            <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
                <div className="sidebar-header">
                    <img src={process.env.PUBLIC_URL + "/logo512_d.png"} alt="Service Gateway Logo" className="sidebar-logo" />
                    {isEmployee && (
                        <img src={process.env.PUBLIC_URL + "/agent_sub.png"} alt="Agent Sublogo" className="sidebar-logo-sub" />
                    )}
                </div>
